...
define([
    ...
    asyncTest('call load then change query via "reset" before load finishes', function() {
        ...
        collection.query.request.ajax = dummyAjax;
        (cancelledDfd = collection.load()).then(function(results) {
            // ok(false, 'cancelledDfd should never resolve');
            ok(true,'cancelledDfd resolved');
            /*if (!donezo) {
                donezo = true;
                start();
            }*/
        ...
        $.when(cancelledDfd,newDfd).then(function(r1,r2){
            deepEqual(r1,r2,'cancelledDfd and newDfd have the same results');
            if (!donezo) {
                donezo = true;
                start();
            }
        },function(f1,f2){
            ok(false,'One or more deferreds have been rejected');
            if (!donezo) {
                donezo = true;
                start();
            }
    ...
    asyncTest('calling multiple loads while resetting params in between, results in lost deferreds',function(){
        var timeout = 1500;
        ...
        collection.reset(params);
        dfd2 = collection.load();
        dfd3 = collection.load();

        timer = setTimeout(function(){
            ok(false,'Timed out before the deferreds resolved');
            console.log(dfd1.state(),dfd2.state(),dfd3.state());
            start();
        },timeout);
        $.when(dfd1,dfd2,dfd3).done(function(){
            clearTimeout(timer);
            ok(true,'All deferreds resolved');
            start();
        });
    });
    ...
    module("caching for contiguous load params");
    var requestCount;
    var thisData = {
        total: 10,
        resources: _(10).range().map(function(i) {
    ...
    var thisRequest = function(params) {
        var limit = params.data.limit,
            offset = params.data.offset,
            newData = $.extend(true, {}, data);
        if (limit) {
            newData.resources = newData.resources.slice(offset, offset + limit);
        }
        requestCount++;
        params.success(newData, 200, {});
    };
    asyncTest('simple load', function() {
        var thisCollection = Example.collection({
                limit: 3,
                offset: 0
            }),
            dfd1, dfd2,
            total;
        thisCollection.query.request.ajax = thisRequest;
        requestCount = 0;
        thisCollection.load().done(function(models) {
            total = thisCollection.total;
            equal(models.length,3,'`models.length` is correct')
            equal(thisCollection.models.length,3,'`collection.models.length` is correct');
            thisCollection.load({limit:5}).done(function(models){
                equal(models.length,2,'`models.length` is correct')
                equal(thisCollection.models.length,5,'`collection.models.length` is correct');
                thisCollection.load({limit:7}).done(function(models){
                    equal(models.length,2,'`models.length` is correct')
                    equal(thisCollection.models.length,7,'`collection.models.length` is correct');
                    thisCollection.load({limit:total+1}).done(function(models){
                        equal(models.length,total-7,'`models.length` is correct')
                        equal(thisCollection.models.length,total,'`collection.models.length` is correct');
                        thisCollection.load({limit:5}).done(function(models){
                            equal(models.length,5,'`models.length` is correct');
                            equal(requestCount,4,'caching still works');
                            thisCollection.load({limit:3}).done(function(models){
                                equal(models.length,3,'`models.length` is correct');
                                equal(requestCount,4,'caching still works');
                                start();
                            });
    ...
    asyncTest('reload should disable caching', function() {
        var thisCollection = Example.collection({
                limit: 3,
                offset: 0
            }),
        ...
        thisCollection.load().done(function(models) {
            ...
            equal(models.length,3,'`models.length` is correct')
            equal(thisCollection.models.length,3,'`collection.models.length` is correct');
            thisCollection.load({limit:5,reload:true}).done(function(models){
                equal(models.length,5,'`models.length` is correct')
                equal(thisCollection.models.length,5,'`collection.models.length` is correct');
                start();
            });
    ...
    asyncTest('non contiguous loads should not break', function() {
        var thisCollection = Example.collection({
                limit: 3,
                offset: 0
            }),
        ...
        thisCollection.load().done(function(models) {
            ...
            equal(models.length,3,'`models.length` is correct')
            equal(thisCollection.models.length,3,'`collection.models.length` is correct');
            thisCollection.load({offset:5,limit:5}).done(function(models){
                equal(models.length,5,'`models.length` is correct')
                equal(_.compact(thisCollection.models).length,8,'compact `collection.models.length` is correct');
                equal(thisCollection.models.length,10,'`collection.models.length` is correct');
                start();
            });
        });
    });
